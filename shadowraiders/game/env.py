...
shadow_characters = [
    'Ulrich:S:11:lie',
    'Vampire:S:11:bloodsucking',
    'Werewolf:S:11:counterattack'
    ]

raider_characters = [
    'Emi:R:11:teleport',
    'Felix:R:11:astralmace',
    'Gordon:R:11:invinciblebarrier'
    ]

citizen_characters = [
    'Alice:C:8:rabbitheal',
    'Daniel:C:11:exclamation'
]
...
#         print(f"***game reset***")
            ...
            if not player.death:
                while(True): # 封印の知恵を引いた場合の処理を行うためのループ
                    ...
                    if not self.win_lose(self.players[0]):
                        ...
                        if target and self.is_value_attack(target):
    ...
    # targetに攻撃できるか
    # 全てのエリアのプレイヤに攻撃可能
    def is_value_attack(self, target):
        if target.death:
            if self.verbose:
    ...
    def attack(self, player, target):
        ...
        damage = 1 + self.black.attack_damage(player)
